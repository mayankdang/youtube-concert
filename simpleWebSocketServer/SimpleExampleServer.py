...
class SimpleChat(WebSocket):
    ...
    def handleMessage(self):
        try:
            userMessageIdMap = json.loads(self.data)
            userId = str(userMessageIdMap.get("id"))
            message = str(userMessageIdMap.get("message"))
            print "userId:", userId
            print "message:", message

            if message.startswith("HELLO_BUDDY"):
                self.sendMessage("HEY_BUDDY:" + message.split(':')[1])
            elif message.startswith("NETWORK_DELAY"):
                delay = int(message.split(':')[1])
                self.sendMessage("******************************HEY_NETWORK!:" + str(delay))
                print "delay:", delay
                ...
                # print userIdMainMap[userId].userId
                # print userIdMainMap[userId].client.address

                self.sendMessage('HeyYo! to self')
                # clients[0].sendMessage("array wala send message")
                # print type(userIdMainMap[userId].client)
                print type(self)
...
if __name__ == "__main__":
    ...
    parser.add_option("--port", default=PORT, type='int', action="store", dest="port", help="port (" + str(PORT) + ")")
